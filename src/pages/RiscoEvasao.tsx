...
const RiscoEvasao: React.FC = () => {
  ...
  // Esta página agora é uma aba dentro da seção Rematrícula (junto com
  // Alunos), então o risco é calculado só sobre os alunos dessa área —
  // mesmo escopo aplicado em Alunos.tsx — em vez de misturar com
  // Retenção/Engajamento.
  const alunosRematricula = useMemo(
    () => alunos.filter((aluno) => aluno.area === "rematricula"),
    [alunos]
  );

  const riscos = useMemo(() => calcularRiscoLista(alunosRematricula), [alunosRematricula]);
